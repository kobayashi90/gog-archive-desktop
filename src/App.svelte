<script>
  ...
  let rawStatuses = [];
  let slugRateInfo = {};
  let rawDownSpeed = 0, rawUpSpeed = 0;
  let smoothDownSpeed = 0, smoothUpSpeed = 0;
  let slugSmoothRate = {};
  const RATE_ALPHA = 0.3;

  function getEstimatedProgress(slug) {
    const info = slugRateInfo[slug];
    if (!info) return 0;
    if (info.progress >= 1) return 1;
    const elapsed = (performance.now() - info.time) / 1000;
    if (info.rate <= 0 || info.total <= 0) return info.progress;
    return Math.min(info.progress + (info.rate * elapsed) / info.total, 1);
  }

  function applySmoothProgress() {
    // Smooth aggregate speeds via EMA
    if (smoothDownSpeed === 0 && rawDownSpeed > 0) smoothDownSpeed = rawDownSpeed;
    if (smoothUpSpeed === 0 && rawUpSpeed > 0) smoothUpSpeed = rawUpSpeed;
    smoothDownSpeed += (rawDownSpeed - smoothDownSpeed) * RATE_ALPHA;
    smoothUpSpeed += (rawUpSpeed - smoothUpSpeed) * RATE_ALPHA;
    downSpeed = smoothDownSpeed;
    upSpeed = smoothUpSpeed;

    // Smooth per-torrent rates and build statuses
    torrentStatuses = rawStatuses.map(s => {
      const raw = s.download_rate;
      if (slugSmoothRate[s.slug] === undefined) slugSmoothRate[s.slug] = raw;
      slugSmoothRate[s.slug] += (raw - slugSmoothRate[s.slug]) * RATE_ALPHA;
      return { ...s, progress: getEstimatedProgress(s.slug), download_rate: slugSmoothRate[s.slug] };
    });

    rafId = requestAnimationFrame(applySmoothProgress);
  }

  onMount(() => {
    appWindow = getCurrentWindow();
    loadFilters();
    trayListener = listen("tray-action", (e) => {
    ...
    torrentListener = listen("torrent-status", (e) => {
      const statuses = e.payload;
      rawStatuses = statuses;
      for (const s of statuses) {
        slugRateInfo[s.slug] = { progress: s.progress, rate: s.download_rate, total: s.total_size, time: performance.now() };
      }
      ...
      rawDownSpeed = downloading.reduce((a, s) => a + s.download_rate, 0);
      rawUpSpeed = statuses.reduce((a, s) => a + s.upload_rate, 0);
      const seeding = statuses.filter((s) => s.state === "seeding" || s.state === "finished");
      activeSeeds = seeding.length;
    });
    rafId = requestAnimationFrame(applySmoothProgress);
  });

  onDestroy(() => {
    if (trayListener) trayListener();
    if (torrentListener) torrentListener();
  ...
  $: activeItems = torrentStatuses.filter(s => s.state !== "paused" && s.state !== "stopped" && s.state !== "error");
  $: anyRunning = torrentStatuses.length > 0 && torrentStatuses.some(t => t.state !== "paused" && t.state !== "stopped" && t.state !== "error" && t.state !== "seeding" && t.state !== "finished");
  $: tbarItem = activeItems[0] || null;
  ...
  async function loadFilters() {
    try { await invoke("get_filters"); } catch (e) {}
  }

  function handleGameCount(e) { gameCount = e.detail; }

  function handleViewGame(e) {
    detailSlug = e.detail;
  ...
  function closeDetail() { detailSlug = null; detailGame = null; }

  function handleNavigateTo(e) { tab = e.detail; }
...
<div class="app">
  <div class="titlebar" data-tauri-drag-region>
    ...
    <div class="titlebar-controls">
      ...
      <button class="titlebar-btn titlebar-close" on:click={closeWindow} aria-label="Close">
  ...
  <header>
    <div class="header-inner">
      <button type="button" class="logo-btn" on:click={goBrowse} aria-label="GOG Archive">
      ...
      <div class="nav-tabs">
        {#each tabs as t}
          <button class="nav-tab" class:active={tab === t.id} on:click={() => { closeDetail(); tab = t.id; }}>
            {#if t.icon === "grid"}
      ...
      {#if tab === "browse"}
        <div class="search-wrap">
          ...
          <input type="text" id="search" placeholder="Search games..." autocomplete="off" bind:value={rawQuery} on:input={onSearchInput} />
          <button class="adv-btn" on:click={handleAdvancedClick} title="Advanced filters">
    ...
    <div class="main-inner">
      <div class:hidden={tab !== "browse"}>
        <Browse
          ...
          on:gameCount={handleGameCount}
          on:viewGame={handleViewGame}
          on:clearSearch={handleClearSearch}
          on:clearAdvFilters={handleClearAdvFilters}
          on:filterGenre={(e) => { advFilters = { ...advFilters, genre: [e.detail] }; tab = "browse"; }}
        />
      </div>
      <div class:hidden={tab !== "queue"}>
        <Queue {torrentStatuses} on:viewGame={handleViewGame} />
      </div>
      <div class:hidden={tab !== "library"}>
        <Library on:viewGame={handleViewGame} on:libraryCount={(e) => libraryCount = e.detail} />
      </div>
  ...
  <div class="torrent-bar">
    <div class="torrent-bar-inner">
      <div class="tbar-center">
        ...
        {:else}
          <div class="tbar-item" on:click={() => (tab = "queue")} on:keydown={(e) => e.key === 'Enter' && (tab = "queue")} role="button" tabindex="0" title="{tbarItem.title || tbarItem.name} &mdash; {(tbarItem.progress * 100).toFixed(1)}%  ETA: {fmtEta(tbarItem.eta)}">
            <img class="tbar-item-thumb" src={tbarItem.image || ""} alt="" loading="lazy" on:error={(e) => e.target.style.display = "none"} />
      ...
      <div class="tbar-right">
        <button class="tbar-btn" title={anyRunning ? "Pause all" : "Resume all"} on:click={toggleAll}>
          {#if anyRunning}
...
{#if detailGame}
  <GameModal game={detailGame} on:close={closeDetail} on:navigateTo={handleNavigateTo} on:filterGenre={(e) => { advFilters = { ...advFilters, genre: [e.detail] }; tab = "browse"; closeDetail(); }} on:filterTag={(e) => { advFilters = { ...advFilters, tag: [e.detail] }; tab = "browse"; closeDetail(); }} on:filterDeveloper={(e) => { advFilters = { ...advFilters, developer: [e.detail] }; tab = "browse"; closeDetail(); }} on:filterPublisher={(e) => { advFilters = { ...advFilters, publisher: [e.detail] }; tab = "browse"; closeDetail(); }} on:filterYear={(e) => { advFilters = { ...advFilters, year: [e.detail] }; tab = "browse"; closeDetail(); }} />
{/if}

{#if showAdvSearch}
          <AdvancedSearch currentFilters={advFilters} on:apply={handleAdvSearch} on:close={() => (showAdvSearch = false)} />
{/if}
