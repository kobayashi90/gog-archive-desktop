<script>
  ...
  function handleOverlayClick(e) { if (e.target === e.currentTarget || e.target.classList.contains('modal-backdrop')) close(); }
  ...
  $: letter = game.title ? game.title[0].toUpperCase() : "?";
</script>
    ...
    <div class="detail-header">
      <div class="detail-cover" class:no-image={!game.image || imageError}>
        {#if game.image && !imageError}
          <img src={game.image} alt={game.title} on:error={handleImageError} />
        {/if}
      ...
      <div class="detail-info">
        ...
        <div class="dev-pub">
          {#if game.developer}
            <button type="button" class="dev-link" on:click={() => dispatch("filterDeveloper", game.developer)}>{game.developer}</button>
          {/if}
          {#if game.developer && game.publisher} &middot; {/if}
          {#if game.publisher}
            <button type="button" class="pub-link" on:click={() => dispatch("filterPublisher", game.publisher)}>{game.publisher}</button>
          {/if}
          {#if game.release_date}
            &middot; <button type="button" class="year-link" on:click={() => dispatch("filterYear", game.release_date.slice(0, 4))}>{game.release_date.slice(0, 4)}</button>
          {/if}
        </div>
        ...
        {#if genres.length > 0 || tags.length > 0}
          <div class="tag-group">
            {#each genres as g}
              <button type="button" class="tag genre" on:click={() => dispatch("filterGenre", g)}>{g}</button>
            {/each}
            {#each tags as t}
              <button type="button" class="tag tag-item" on:click={(e) => { e.stopPropagation(); dispatch("filterTag", t); }}>{t}</button>
            {/each}
          </div>
        {/if}
      </div>
      ...
      {#if game.magnet_link}
        <div class="detail-section">
          ...
          {:else if addStatus === "error"}
            ...
            <button class="download-torrent-btn" on:click={startDownload}>
            ...
            <div class="torrent-file-select">
              <div class="file-select-header">
                <label class="toggle-all">
                  <input type="checkbox" checked={selectedFiles.size === torrentFiles.length} on:change={toggleAll} />
              ...
              <div class="torrent-files-list">
                {#each torrentFiles as f}
                  <label class="torrent-file-item" class:selected={selectedFiles.has(f.index)}>
                    <input type="checkbox" checked={selectedFiles.has(f.index)} on:change={() => toggleFile(f.index)} />
              ...
              <div class="file-select-actions">
                <button class="download-torrent-btn" on:click={startDownload} disabled={installing || selectedFiles.size === 0}>
                ...
                <button class="cancel-select-btn" on:click={() => { showFileSelector = false; }}>Cancel</button>
              </div>
            </div>
          {:else if previewing}
          ...
          {:else}
            <button class="download-torrent-btn" on:click={download} disabled={installing}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/>
                <polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/>
              </svg>
      ...
      {#if files.length > 0}
        <div class="detail-section">
          ...
          {#if hiddenFiles().length > 0}
            ...
            <button class="files-toggle" on:click={() => (showAllFiles = !showAllFiles)}>
      ...
      {#if game.gog_url || game.gogdb_url || game.pcgamingwiki_url}
        <div class="detail-section links-section">
          <div class="link-group">
            {#if game.gog_url}
              <button type="button" class="ext-link" on:click={() => invoke("open_url", { url: game.gog_url })}>GOG</button>
            {/if}
            {#if game.gogdb_url}
              <button type="button" class="ext-link" on:click={() => invoke("open_url", { url: game.gogdb_url })}>GOGDB</button>
            {/if}
            {#if game.pcgamingwiki_url}
              <button type="button" class="ext-link" on:click={() => invoke("open_url", { url: game.pcgamingwiki_url })}>PCGamingWiki</button>
            {/if}
          </div>
        </div>
      {/if}
