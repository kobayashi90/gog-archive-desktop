<script>
  ...
  async function poll() {
    try {
      ...
      onlibraryCount?.(allEntries.length);
    } catch (e) {}
  }
    ...
    <div class="grid">
      {#each filteredEntries as e}
        <div class="card">
          ...
          <div class="actions">
            <button class="act-btn" onclick={() => openFolder(e.slug)} title="Open folder">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M22 19a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2h5l2 3h9a2 2 0 012 2z"/>
              </svg>
            </button>
            <button class="act-btn danger" onclick={() => confirmDelete(e)} title="Delete" disabled={deleting === e.slug}>
              {#if deleting === e.slug}
