<script>
  ...
  $: active = torrents.filter((t) => t.state !== "seeding" && t.state !== "finished" && (t.progress || 0) < 100);
  $: completed = torrents.filter((t) => t.state === "seeding" || t.state === "finished");
</script>
      ...
      <div class="queue-list">
        {#each active as t}
          <div class="queue-item">
            {#if t.image}
              <img class="queue-thumb" src={t.image} alt="" loading="lazy" on:error={(e) => e.target.style.display = "none"} />
            {:else}
            ...
            <div class="queue-actions">
              {#if t.state === "paused" || t.state === "error" || t.state === "stopped"}
                <button class="queue-btn" on:click={() => resume(t.slug)}>Resume</button>
              {:else}
                <button class="queue-btn" on:click={() => pause(t.slug)}>Pause</button>
              {/if}
              <button class="queue-btn danger" on:click={() => confirmRemove(t)} disabled={removing === t.slug}>
                {#if removing === t.slug}
      ...
      <div class="queue-list">
        {#each completed as t}
          <div class="queue-item done">
            ...
            <div class="queue-actions">
              <button class="queue-btn" on:click={() => openFolder(t.slug)} title="Open folder">
